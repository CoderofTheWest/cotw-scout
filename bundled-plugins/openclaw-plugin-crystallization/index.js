...
/**
 * DANGEROUS: Write permanent trait to traits.json.
 * This permanently modifies Clint's character — the trait becomes part of identity.
 */
async function DANGEROUS_persistTrait(traitsPath, traitRecord, _reason) {
...
/**
 * DANGEROUS: Update growth vectors with crystallization status.
 * Marks vectors as approved/pending — feeds into identity evolution.
 */
async function DANGEROUS_updateVectorStatus(vectorsPath, updatedVectors, _reason) {
...
function createPlugin(api, userConfig = {}) {
  ...
  async function runCrystallization(candidateIds = null) {
    ...
    await DANGEROUS_updateVectorStatus(vectorsPath, updatedVectors, 'crystallization candidate proposed — pending review');
  ...
  async function onAgentEnd(context = {}) {
    ...
    if (decision === 'approve') {
      ...
      await DANGEROUS_persistTrait(traitsPath, traitRecord, 'user approved crystallization');
      await DANGEROUS_updateVectorStatus(vectorsPath, approvedVectors, 'user approved crystallization');
    }
  }

  // Load-order race with nightshift plugin — see contemplation plugin
  // for the same fix. One-shot check silently skips when nightshift
  // loads later; poll for availability instead.
  //
  // Reference correction (2026-04-20): crystallization previously looked
  // for api.nightshift, but OpenClaw's buildPluginApi creates per-plugin
  // api objects — api.nightshift is only set on the nightshift plugin's
  // own api. global.__ocNightshift is the truly shared reference (the
  // nightshift plugin assigns both: `api.nightshift = nightshiftApi;
  // global.__ocNightshift = nightshiftApi;` at line 410-411 of its
  // register()). Crystallization's task runner was never registering
  // before this fix — runtime log confirmed silent failure. Clint
  // diagnosed the same bug on macMini.
  const registerCrystallizationTaskRunner = () => {
    ...
    if (api?.logger?.info) {
      api.logger.info('[Crystallization] Registered nightshift task runner');
    }
  };

  if (global.__ocNightshift?.registerTaskRunner) {
    registerCrystallizationTaskRunner();
  } else {
    ...
    const nightshiftPollId = setInterval(() => {
      ...
      if (global.__ocNightshift?.registerTaskRunner) {
        ...
        clearInterval(nightshiftPollId);
        if (api?.logger?.warn) {
          api.logger.warn('[Crystallization] Nightshift never loaded after 60s — crystallization task runner NOT registered. Check plugin load order.');
        }
      }
    }, 2000);
  }
...
module.exports = {
  ...
  register(api) {
    api = instrumentApiHooks(api, 'crystallization');
    const userConfig = api.pluginConfig || {};
    const plugin = createPlugin(api, userConfig);

    // Register hooks
    api.on('agent_end', async (event, ctx) => {
      // On heartbeat-triggered turns, run auto-crystallization
      // (heartbeat is a trigger type, not a hook name — call runCrystallization directly)
      if (ctx.trigger === 'heartbeat') {
        await plugin.runCrystallization();
        return;
      }
      // On normal turns, check for approval decisions
      await plugin.hooks.agent_end(ctx);
    });
