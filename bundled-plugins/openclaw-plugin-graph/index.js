...
module.exports = {
    ...
    register(api) {
        api = instrumentApiHooks(api, 'graph');
        ...
        // -----------------------------------------------------------------
        // ResearchGraph narrow canary bridge
        // -----------------------------------------------------------------
        // Bridge through the already-loaded graph plugin so broad/general
        // injection remains classifier-blocked and rollback is one hunk.
        if (config.researchGraphCanary?.enabled === true) {
            try {
                const researchGraphPlugin = require('../openclaw-plugin-research-graph');
                const researchApi = Object.create(api);
                researchApi.pluginConfig = config.researchGraphCanary;
                researchGraphPlugin.register(researchApi);
                api.logger.info('[Graph] ResearchGraph narrow canary bridge registered');
            } catch (err) {
                api.logger.warn(`[Graph] ResearchGraph bridge disabled: ${err.message}`);
            }
        }
        ...
        setImmediate(() => {
            ...
            if (global.__ocMetabolism?.registerGapListener) {
                global.__ocMetabolism.registerGapListener('graph', (gaps, agentId) => {
                    api.logger.debug(
                ...
                api.logger.info('[Graph] Subscribed to legacy metabolism gap events');
            }
            ...
            global.__ocNightshift.registerTaskRunner('graph-enrichment', async (task, ctx) => {
                ...
                if (gaps.length > 0 && (global.__ocMetabolism?.emitGaps || global.__ocMetabolism?.gapListeners)) {
                    // Push graph gaps through the same pipeline contemplation uses
                    const listeners = global.__ocMetabolism.emitGaps
                        ? [(items, agentId) => global.__ocMetabolism.emitGaps(items, agentId)]
        ...
        if (global.__ocMetabolism?.emitGaps || global.__ocMetabolism?.gapListeners) {
