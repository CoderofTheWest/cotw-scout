...
## Source-addressable claims operator helpers

The claims surface is deliberately operator-led. It can show claim records,
attached source handles, and bounded source comparisons, but it does not consume
claim context automatically or promote claims without a separate explicit
decision workflow.

### Inspect claims and attached handles

Use the registered command `continuity-claims` to find exact claim ids and
source handles before any source-resolution work. In chat/TUI command surfaces,
invoke it as:

```text
/continuity-claims verify --include-sources --limit 10
/continuity-claims get --claim-id <claim-id> --include-sources
```

This command is read-only. Even with `--include-sources`, it only reports stored
handles and metadata; it does not resolve source text.

### Resolve one source handle

Use `continuity-claims-source` when the operator needs to inspect bounded source
content behind one exact claim or source handle:

```text
/continuity-claims-source resolve --claim-id <claim-id> --source-handle <attached-source-handle> --max-content-chars 1200
```

The source handle must already be attached to the claim. Resolution is bounded
and read-only; it does not verify, promote, mutate, consume, or inject prompt
context.

### Compare one claim against one attached source

Use `continuity-claims-verify-source` for the Build 5 verification helper:

```text
/continuity-claims-verify-source compare --claim-id <claim-id> --source-handle <attached-source-handle> --max-content-chars 1200
```

Optional flags:

- `--claim-text` — include the claim text in the receipt.
- `--metadata` — include source-resolution metadata when available.
- `--max-content-chars <n>` — bound resolved source content between 80 and 4000
  characters. Default is 800.

The helper emits lexical guidance only:

- `source_contains_claim_text` — normalized source text contains the normalized
  claim text.
- `source_likely_supports_claim` — high token overlap; operator review is still
  required.
- `source_partially_overlaps_claim` — do not promote without additional evidence
  or a narrower claim.
- `source_does_not_show_enough_overlap` — do not promote from this source alone.

Every receipt also states the held boundaries:
...
Mutation-like flags such as `--apply`, `--promote`, `--verify`, `--consume`,
`--inject`, `--accept`, and `--accept-verified` are rejected here. If the helper
shows enough evidence, use a separate explicit decision workflow rather than
treating this helper as the decision.

### Accept a verified claim as a separate operator decision

Use `continuity-claims-decision accept_verified` only after inspecting the claim
and source through the read-only surfaces above. This decision command is dry-run
by default:

```text
/continuity-claims-decision accept_verified --claim-id <claim-id> --source-handle <attached-source-handle> --accepted-staleness-policy <policy> --reason "<operator reason>"
```

A dry run may plan activation, but it must not mutate or promote the claim. The
only promotion path is the same explicit decision with `--apply` added:

```text
/continuity-claims-decision accept_verified --claim-id <claim-id> --source-handle <attached-source-handle> --accepted-staleness-policy <policy> --reason "<operator reason>" --apply
```

Required gates:

- exact claim id
- exact attached source handle
- operator reason
- accepted staleness policy
- explicit `--apply` for mutation or promotion

Do not combine comparison and promotion. `continuity-claims-verify-source` gives
operator guidance; `continuity-claims-decision accept_verified` records the
operator decision.
