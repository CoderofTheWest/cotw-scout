 ...
 * Deduplication key: `${timestamp}_${sender}_${contentHash}` prevents double-archiving.
 * Secondary dedup: same sender + same content within 60s catches retried messages.
 */
 ...
 * Not cryptographic — used for dedup fingerprinting only.
 */
    ...
    archive(messages, opts = {}) {
        ...
        const normalized = messages
            ...
            .map(m => ({
                ...
                entropyLevel: opts.entropyLevel || null
            }));
        ...
        for (const [date, dayMessages] of byDate) {
            ...
            // Build dedup set from existing messages (content-aware keys)
            const dedupKeys = new Set(
                existing.messages.map(m => {
                    const hash = m.contentHash || fnv1aHash(m.text || '');
                    return `${m.timestamp}_${m.sender}_${hash}`;
                })
            );
            ...
            for (const msg of dayMessages) {
                const contentHash = fnv1aHash(msg.text || '');
                msg.contentHash = contentHash;
                const key = `${msg.timestamp}_${msg.sender}_${contentHash}`;
                if (!dedupKeys.has(key)) {
                    // Secondary check: same sender + same content within 60s = retry duplicate
                    const isRetryDuplicate = existing.messages.some(m => {
                        const mHash = m.contentHash || fnv1aHash(m.text || '');
                        if (m.sender !== msg.sender || mHash !== contentHash) return false;
                        const timeDiff = Math.abs(new Date(m.timestamp) - new Date(msg.timestamp));
