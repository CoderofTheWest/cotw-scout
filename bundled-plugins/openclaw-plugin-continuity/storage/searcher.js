    ...
    async search(query, limit = 5, agentId, scope = null, threadId = null) {
        ...
        try {
            ...
            for (const [id, rrfScore] of rrfScores) {
                ...
                // Emotional bleed prevention: suppress elevated-entropy exchanges
                // when the current conversation is task-focused (low entropy), and
                // mildly boost them when the current conversation is relational.
                const exEntropyLevel = ex.entropy_level || ex.entropyLevel
                    || (meta?.entropyLevel) || null;
                if (exEntropyLevel && scope?.currentEntropy !== undefined) {
                    const isEmotional = exEntropyLevel === 'elevated' || exEntropyLevel === 'critical';
                    if (isEmotional && scope.currentEntropy < 0.3) {
                        compositeScore *= 0.3; // Heavy penalty — suppress emotional context during task work
                    } else if (isEmotional && scope.currentEntropy > 0.4) {
                        compositeScore *= 1.2; // Mild boost — surface relevant emotional history
                    }
                }
                ...
                // senderFilter: strip agent text when only user messages are safe
                // (used during restart quarantine to prevent feedback loop)
                if (scope?.senderFilter === 'user') {
                    result.agentText = null;
                }
