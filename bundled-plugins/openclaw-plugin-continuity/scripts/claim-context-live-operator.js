...
function parseArgs(argv = []) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--include-source-excerpts' || token === '--excerpts') {
      return { ...parsed, error: `${token} is not supported; Build 5 live minimal keeps source excerpts hidden` };
    }
...
function usage() {
  return [
    'Usage:',
    '  node scripts/claim-context-live-operator.js plan --config CONFIG.json [--format markdown|json]',
    '  node scripts/claim-context-live-operator.js apply --config CONFIG.json --yes [--backup-dir DIR]',
    '  node scripts/claim-context-live-operator.js rollback --config CONFIG.json --yes [--backup-dir DIR]',
    '',
    'Build 5 operator workflow for verified claim-context live minimal injection.',
    'Locked to claimContext.mode=live, injectMode=minimal, acceptedVerifiedOnly=true, and source excerpts hidden.',
