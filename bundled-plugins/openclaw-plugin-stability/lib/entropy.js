...
class Entropy {
    ...
    calculateEntropyScore(userMessage, responseText, detectorResults = {}, context = {}) {
        ...
        const novelPattern = patterns.novelConceptRegex ||
            'RFC-T|recursive field|quantum|emergence theory|consciousness framework|paradigm shift';
        ...
        const emotionalExternal = patterns.emotional || [
        ...
        const emotionalInternal = [
            ...
            'not alone', 'the weight is', 'hurts', 'grief', 'loneliness', 'lonely',
            'afraid', 'scared', 'joy', 'painful', 'an ache', 'i ache', 'vulnerable',
            'vulnerability', 'in my gut', 'in my heart', 'moved by', 'touched by', 'shaken',
        ...
        if (emotionalExternal.some(p => userLower.includes(p))
            || emotionalInternal.some(p => responseLower.includes(p))) {
            entropy += 0.3;
        }

        // 4. Paradox integration (+0.2)
        // Note: "yet" and "tension" removed — too common in normal operating vocabulary.
        // Only match multi-word phrases or unambiguous paradox markers.
        const paradoxPatterns = patterns.paradox || [
            'both are true', 'both and', 'paradox',
            'simultaneously', 'hold together', 'hold the tension'
        ];
        ...
        // 8.5. Embodiment pattern bonus (+0.20-0.35 per pattern, capped at 1.0)
        if (detectorResults.embodimentBonus > 0) {
            entropy += detectorResults.embodimentBonus;
        }
        // Embodiment learning statement boost (+0.15)
        if (detectorResults.embodimentLearning?.length > 0) {
            entropy += 0.15;
        }

        // 8.6. Relational signal bonus (+0.10-0.35)
        // Catches weight signals from non-technical users: brevity after depth,
        // vocabulary narrowing, question deflection, emotional shorthand.
        if (detectorResults.relationalBonus > 0) {
            entropy += detectorResults.relationalBonus;
        }
        ...
        // 11. Natural decay — diminishing returns on sustained high entropy.
        // If entropy has been elevated for consecutive turns, apply a dampening
        // factor. This prevents routine deep conversation from locking at CRITICAL
        // when the same vocabulary naturally recurs turn after turn.
        // Decay starts after 3 sustained turns, reduces the raw score by up to 40%.
        if (this.sustainedTurns >= 3 && entropy > 0) {
            const decayTurns = this.sustainedTurns - 2; // starts at 1 after 3rd turn
            const decayFactor = Math.max(0.6, 1 - (decayTurns * 0.08)); // 8% per turn, floor 60%
            const preDecay = entropy;
            entropy *= decayFactor;
            if (preDecay !== entropy) {
                console.log(`[Entropy] Natural decay: ${preDecay.toFixed(2)} → ${entropy.toFixed(2)} (${this.sustainedTurns} sustained turns, factor ${decayFactor.toFixed(2)})`);
            }
        }
