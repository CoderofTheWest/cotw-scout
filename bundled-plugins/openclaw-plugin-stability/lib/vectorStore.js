 ...
 *
 * Memory type taxonomy (March 31, 2026 — KAIROS-inspired):
 *   user       — about Chris (preferences, role, knowledge)
 *   feedback   — corrections + confirmed approaches (Rule → Why → How to apply)
 *   project    — work context not derivable from code (deadlines, incidents)
 *   reference  — pointers to external systems (dashboards, boards, channels)
 *   embodiment — physical knowledge from TonyPi (motor strategies, calibration)
 *
 * Design principles:
...
const VALID_VECTOR_TYPES = ['user', 'feedback', 'project', 'reference', 'embodiment'];
...
class VectorStore {
    ...
    formatForInjection(vectors) {
        ...
        const lines = ['[PATTERNS YOU\'RE DEVELOPING]'];
        for (const v of vectors) {
            const type = v.type || 'project';
            const hypothesis = v.integration_hypothesis || v.description || v.text || 'unspecified';
            const weight = (v.weight || 0.5).toFixed(2);
            lines.push(`[${type}] ${hypothesis} (w:${weight})`);
        }
    ...
    /**
     * DANGEROUS: Add an auto-detected candidate vector.
     * May auto-promote to permanent vectors array if recurrence >= threshold.
     */
    DANGEROUS_addCandidate(candidate, _reason = 'auto-detected') {
        try {
            // Normalize type to taxonomy
            if (candidate.type && !VALID_VECTOR_TYPES.includes(candidate.type)) {
                // Map legacy types to taxonomy
                const legacyMap = {
                    'embodiment_learning': 'embodiment',
                    'user_correction': 'feedback',
                    'procedural': 'feedback',
                    'pattern_recognition': 'project',
                    'preference_learning': 'user',
                    'insight': 'project'
                };
                candidate.type = legacyMap[candidate.type] || 'project';
            }
            if (!candidate.type) candidate.type = 'project';

            const data = this._readFileRaw();
            if (!data.candidates) data.candidates = [];

            // Check for duplicate (same type + similar description OR similar hypothesis)
            const isDuplicate = data.candidates.some(c =>
                c.type === candidate.type && (
                    this._similarity(c.description, candidate.description) > 0.7 ||
                    this._similarity(c.integration_hypothesis || '', candidate.integration_hypothesis || '') > 0.7
                )
            );
    ...
    recordFeedback(vectorId, feedback) {
        ...
        try {
            ...
            // Auto-retire ineffective vectors: >20 injections with negligible entropy impact
            if (record.totalInjections > 20 && Math.abs(record.avgEntropyDelta) < 0.05) {
                this._retireVector(vectorId, `Auto-retired: ${record.totalInjections} injections with avg delta ${record.avgEntropyDelta.toFixed(3)}`);
                console.log(`[Stability/VectorStore] Auto-retired ineffective vector ${vectorId} (${record.totalInjections} injections, avgDelta=${record.avgEntropyDelta.toFixed(3)})`);
            }
    ...
    /**
     * Retire an ineffective vector — remove from active vectors array.
     * Retired vectors are moved to a 'retired' array for audit trail.
     */
    _retireVector(vectorId, reason) {
        try {
            const data = this._readFileRaw();
            if (!data.vectors) return;

            const idx = data.vectors.findIndex(v => v.id === vectorId);
            if (idx < 0) return;

            const vector = data.vectors.splice(idx, 1)[0];
            vector.validation_status = 'retired';
            vector.retired_at = new Date().toISOString();
            vector.retired_reason = reason;

            if (!data.retired) data.retired = [];
            data.retired.push(vector);

            this._writeFile(data);
        } catch (err) {
            console.warn(`[Stability/VectorStore] Failed to retire vector ${vectorId}:`, err.message);
        }
    }
