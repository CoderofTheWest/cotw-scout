...
module.exports = async function agent_end({ messages, metadata, memory }) {
    ...
    // 1. Run detectors (pass previous entropy for relational signal detection)
    const previousEntropy = entropy.lastScore || 0;
    const detectorResults = detectors.runAll(userMessage, responseText, previousEntropy);
