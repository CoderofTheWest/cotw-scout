...
function validateEvolutionAction({ action, entry }) {
  ...
  if (action === 'apply_scaffold_proposal' && entry) {
    ...
    if (entry.action !== 'scaffold_proposal') blockers.push('scaffold_apply_requires_scaffold_proposal');
...
function expectedEffectForAction(action) {
  ...
  if (action === 'apply_scaffold_proposal') return 'Promote one low-risk Code Evolution scaffold proposal into evolved scaffold files with before/after hashes and rollback snapshot.';
