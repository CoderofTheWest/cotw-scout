...
const HARNESS_REFINER_SCAFFOLD_LANES = Object.freeze({
...
function promoteHarnessRefinerProposal(entry = {}, options = {}) {
  const scaffoldEntry = scaffoldProposalFromHarnessRefiner(entry);
  const result = promoteScaffoldProposal(scaffoldEntry, options);
  return {
    ...result,
    sourceProposalAction: 'harness_refinement_proposal',
    harnessLane: entry.metadata?.lane || '',
    targetSurface: entry.metadata?.targetSurface || result.target
  };
}
...
function buildHarnessRefinerPromotionEvent(proposalEntry = {}, result = {}, options = {}) {
  const scaffoldEntry = scaffoldProposalFromHarnessRefiner(proposalEntry);
  const event = buildScaffoldPromotionEvent(scaffoldEntry, result, options);
  const now = options.now || result.promotedAt || new Date().toISOString();
  const promotionId = `harness-refiner-promotion-${safeId(proposalEntry.id)}`;
  const lane = proposalEntry.metadata?.lane || result.harnessLane || '';
  const targetSurface = proposalEntry.metadata?.targetSurface || result.targetSurface || event.metadata.target;
  return {
    ...event,
    id: promotionId,
    title: `Promoted Harness Refiner proposal: ${safe(proposalEntry.title || targetSurface, 140)}`,
    summary: `Applied ${String(result.changeType || event.metadata.changeType).replaceAll('_', ' ')} to ${targetSurface}: ${safe(result.proposedChange || proposalEntry.metadata?.proposedChange || proposalEntry.summary, 420)}`,
    sourceCategory: 'harness-refiner scaffold promotion',
    allowedBy: 'Operator-triggered Evolve promotion for low-risk Harness Refiner workflow/tool-hint proposals through the existing scaffold gate.',
    expectedEffect: proposalEntry.expectedEffect || 'Reduce recurrence of the observed trajectory failure through a bounded scaffold update.',
    verification: `Before hash ${result.beforeHash}; after hash ${result.afterHash}. Harness proposal evidence remains attached to ${proposalEntry.id}.`,
    rollback: 'Use rollback_scaffold_promotion to restore the before-promotion scaffold snapshot.',
    receiptId: promotionId,
    rollbackAction: {
      action: 'rollback_scaffold_promotion',
    ...
    metadata: {
      ...
      originalProposalId: proposalEntry.id,
      sourceProposalAction: 'harness_refinement_proposal',
      sourceProposalKind: proposalEntry.metadata?.signature || proposalEntry.metadata?.proposalKind || '',
      harnessLane: lane,
      targetSurface,
...
function scaffoldProposalFromHarnessRefiner(entry = {}) {
  assertHarnessRefinerPromotionEntry(entry);
  const metadata = entry.metadata || {};
  const lane = metadata.lane;
  const changeType = HARNESS_REFINER_SCAFFOLD_LANES[lane];
  const target = safe(metadata.target || metadata.targetSurface || entry.sourceCategory || 'harness-refiner', 120);
  const proposedChange = safe(metadata.proposedChange || entry.summary, 1000);
  return {
    ...entry,
    action: 'scaffold_proposal',
    sourceCategory: 'harness-refiner scaffold proposal',
    allowedBy: 'Harness Refiner low-risk proposal routed through Code Evolution scaffold promotion gate.',
    metadata: {
      ...metadata,
      proposalKind: metadata.signature || metadata.proposalKind || 'harness_refinement',
      changeType,
      target,
      proposedChange,
      mutationAttempted: 'false',
      promptInjectionChanged: 'false',
      harnessRefinerOriginalAction: entry.action,
      harnessRefinerLane: lane,
      originalHarnessProposalId: entry.id
    }
  };
}
...
function assertHarnessRefinerPromotionEntry(entry = {}) {
  if (entry.action !== 'harness_refinement_proposal') throw new Error('only harness_refinement_proposal receipts can use the Harness Refiner promotion adapter');
  if (entry.status !== 'preview') throw new Error('only preview Harness Refiner proposals can be promoted');
  if (entry.risk !== 'low') throw new Error('only low-risk Harness Refiner proposals can be promoted');
  const metadata = entry.metadata || {};
  const lane = metadata.lane;
  if (!HARNESS_REFINER_SCAFFOLD_LANES[lane]) {
    throw new Error(`unsupported Harness Refiner promotion lane: ${lane || 'unknown'}`);
  }
  if (metadata.applyPath !== 'existing_scaffold_gate') {
    throw new Error('Harness Refiner proposal is not bound to the existing scaffold gate');
  }
  if (metadata.mutationAttempted === true || metadata.mutationAttempted === 'true') {
    throw new Error('proposal already reports mutation; promotion refused');
  }
  if (metadata.promptInjectionChanged === true || metadata.promptInjectionChanged === 'true') {
    throw new Error('proposal reports prompt injection change; promotion refused');
  }
  if (isTrueFlag(metadata.launchTraining) || isTrueFlag(metadata.adapterPromotion) || isTrueFlag(metadata.modelRoutingMutation) || isTrueFlag(metadata.gatewayInvocation)) {
    throw new Error('Harness Refiner scaffold promotion refuses training, adapter, routing, or Gateway mutation flags');
  }
}
...
module.exports = {
  ...
  scaffoldProposalFromHarnessRefiner,
  scaffoldHash
};
