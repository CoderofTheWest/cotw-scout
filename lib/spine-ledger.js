...
const DEFAULT_HOT_LEDGER_LIMITS = {
  outcomeEvents: 400,
  governorDecisions: 400,
  contextEligibilityReviews: 400,
  maturationCandidates: 400,
  responsibilityLeases: 200
};
const HOT_LEDGER_COLLECTIONS = Object.keys(DEFAULT_HOT_LEDGER_LIMITS);
...
function compactSpineLedgerForHotPath(ledger, ledgerPath, options = {}) {
  const limits = { ...DEFAULT_HOT_LEDGER_LIMITS, ...(options.limits || {}) };
  const archiveRecords = [];
  const archivedAt = options.now || new Date().toISOString();
  const next = normalizeLedger(ledger);

  for (const collection of HOT_LEDGER_COLLECTIONS) {
    const limit = Math.max(0, Number.isInteger(limits[collection]) ? limits[collection] : DEFAULT_HOT_LEDGER_LIMITS[collection]);
    const items = Array.isArray(next[collection]) ? next[collection].slice() : [];
    if (items.length <= limit) {
      next[collection] = items;
      continue;
    }
    items.sort((a, b) => String(packetSortKey(b)).localeCompare(String(packetSortKey(a))));
    next[collection] = items.slice(0, limit);
    for (const packet of items.slice(limit)) {
      archiveRecords.push({ archivedAt, collection, packet });
    }
  }

  if (archiveRecords.length && ledgerPath && options.archive !== false) {
    const archivePath = resolveSpineLedgerArchivePath(ledgerPath);
  ...
  return {
    ledger: next,
    compacted: archiveRecords.length > 0,
    archivedCount: archiveRecords.length,
    archivePath: archiveRecords.length && ledgerPath ? resolveSpineLedgerArchivePath(ledgerPath) : null
  };
}

function compactSpineLedgerFile(ledgerPath, options = {}) {
  const ledger = readSpineLedger(ledgerPath);
  const result = compactSpineLedgerForHotPath(ledger, ledgerPath, options);
  if (result.compacted || options.forceWrite) {
    fs.mkdirSync(path.dirname(ledgerPath), { recursive: true });
    writeJsonAtomic(ledgerPath, result.ledger);
  }
  return {
    ...
    counts: {
      outcomeEvents: result.ledger.outcomeEvents.length,
      governorDecisions: result.ledger.governorDecisions.length,
      contextEligibilityReviews: result.ledger.contextEligibilityReviews.length,
      maturationCandidates: result.ledger.maturationCandidates.length,
      responsibilityLeases: result.ledger.responsibilityLeases.length
    }
  };
}

function resolveSpineLedgerArchivePath(ledgerPath) {
  const parsed = path.parse(ledgerPath);
  return path.join(parsed.dir, `${parsed.name}.archive.jsonl`);
}

function packetSortKey(packet = {}) {
  return packet.createdAt
    || packet.lifecycle?.createdAt
    || packet.lifecycle?.updatedAt
    || packet.receipts?.createdByEvent
    || packet.eventId
    || packet.decisionId
    || packet.reviewId
    || packet.candidateId
...
function persistSpineLedger(ledgerPath, ledger) {
  fs.mkdirSync(path.dirname(ledgerPath), { recursive: true });
  const result = compactSpineLedgerForHotPath(normalizeLedger(ledger), ledgerPath);
  writeJsonAtomic(ledgerPath, result.ledger);
  return ledgerPath;
}
